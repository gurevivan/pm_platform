...
`setup.sh` автоматически определяет IP-адрес сервера и добавляет его в `backend/.env` — никаких ручных правок не требуется.

Скрипт также предлагает выбрать Docker-сеть: создать новую `pm_net` или подключиться к уже существующей (полезно, если на сервере запущены другие Docker-проекты и пул адресов исчерпан).

После завершения `setup.sh` приложение будет доступно по адресу `http://IP-АДРЕС-СЕРВЕРА`.

> **Если IP поменялся** или вы добавляете домен — откройте `backend/.env`, добавьте новый адрес в три строки и перезапустите backend:
> ```env
